\section{Hands-Off O2H: A Better O2H Lemma from Compressed Oracles?}\label{sec:handsoff}
  ...
  \subsection{The Hand-Off O2H Lemma}
    \begin{lemma}[Hands-Off O2H]\label{lemma:handsoff-o2h}
  ...
  \subsection{Optimality of the Bound}
    \hhnote{
  ...
  \subsection{Relation to Semi-Classical O2H}
    \hhnote{
      ...
      But \ldots does this have any utility? A: If you can generalize to oracles
      that only differ on points in $\Sspace$ then yes, if nothing else to show
      that this is more general (if it is).
    }
  ...
  \subsection{From Onewayness to Indistinguishability with Optimal Tightness}
    As a proof of concept, \dots
    \hhnote{
      Show that a $\owpca$ KEM gives an $\indcpa$ PKE via the BRE transform,
      tightly up to the square root. (Then generalize to CCA and publish at
      TCC.)
    }
