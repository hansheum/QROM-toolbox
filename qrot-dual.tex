\section{Thinking in Dual Alphabets}\label{sec:dual}
  ...
  \subsection{Properties of $\xstore$ and $\zstore$.}
    \subsubsection{The Single-Qubit Case.}
      ...
      Next up in our table is the multi-alphabet state
        ...
        Well, recall that the $\Xgate$ and $\Zgate$ play opposite roles in the
        two domains, so applying a (controlled) $\Xgate$ gate to $\ket{\hat b}$
        will have the same effect as if we applied a (controlled) $\Zgate$ gate
        to $\ket{b}$: If $b=1$ (so that $\hat b = \hat 1$), then the
        phase is flipped, and otherwise nothing happens.
        Since the operation is further only applied if $a = 1$, we need that
        their \emph{product} is $1$ for the phase to be flipped. In other words,
      ...
      The only possible effect is a flip of the global phase.
        ...
        \emph{identity}.\footnote{
          Another way to say this is that $\ket{\hat a, b}$ is an
          eigenstate of the $\hCZgate$ operator, which actually follows
          trivially from the fact that $\ket b$ is an eigenstate of the $\Zgate$
          operator.
        }
        But of course, we will often \emph{not} be applying $\zstore$ in isolation, and
        ...
        is no longer global, and it may certainly \emph{not} be ignored.

      Using the diagonal symmetry of our table, we may immediately conclude that
        the operation $\register[B] \zstore \register[A]$ with initial state
        $\ket{\hat a, b}$ will have the same net effect 
        (we could also have seen this from \cref{lemma:xzstore}):
      \[
      ...
      Finally, how does $\register[A] \xstore \register[B]$ affect the initial
        state $\ket{\hat a, b}$? This operation should look familiar to you: We
        ...
        usually prepared, with $a=0$, $b=0$ being the canonical choice,
        in which case we end up with the maximally-entangled ``Bell state'' 
        $\nicefrac{1}{\sqrt 2}\, (\ket{00} + \ket{11})$.

      Let's look at the general case:
        register $\register[A]$ is initialized to $\ket{\hat a}$ and register
        $\register[B]$ is initialized to $\ket b$. Then 
        $\register[B] \xstore \register[A]$ acts as follows:
        \[
          \ket{\hat a, b} 
        ...
        The result is one of the four states  of the \emph{Bell basis} $\{
          \ket{\mathrm{\Phi}^+}, \ket{\mathrm{\Phi}^-}, \ket{\mathrm{\Psi}^+},
          \ket{\mathrm{\Psi}^-} \}$, depending on the values of $a$ and $b$:
        \begin{align*}
          (a,b) = (0,0)\ \implies\ \ket{\mathrm{\Phi}^+} \coloneqq \frac{\ket{00} + \ket{11}}{\sqrt 2}\, ,
          &\qquad
          (a,b) = (1,0)\ \implies\ \ket{\mathrm{\Phi}^-} \coloneqq \frac{\ket{00} - \ket{11}}{\sqrt 2}\, , 
          \\
          (a,b) = (0,1)\ \implies\ \ket{\mathrm{\Psi}^+} \coloneqq \frac{\ket{01} + \ket{10}}{\sqrt 2}\, ,
          &\qquad
          (a,b) = (1,1)\ \implies\ \ket{\mathrm{\Psi}^-} \coloneqq \frac{\ket{01} - \ket{10}}{\sqrt 2}\, .
        \end{align*}

      Let's adopt the notation of Nielsen and Chuang~\cite{NieChu10} and write
        $\ket{\beta_{ab}}$ for the Bell state we get when starting from $\ket{\hat a, b}$
        and applying $\register[A] \xstore \register[B]$.
        The same diagonal table-symmetry then tells us that applying
        $\register[B] \zstore \register[A]$ on the initial state $\ket{a, \hat b}$
        must yield the same Bell state, except with hats:
        \[
          \ket{a, \hat b} 
            = \frac{\ket{\hat 0} + (-1)^a \ket{\hat 1}}{\sqrt 2}\ket{\hat b}\ 
            \apply{\hCZgate}\ \frac{\ket{\hat 0, \hat b} + (-1)^a \ket{\hat 1, \hat b \xor \hat 1}}{\sqrt 2}
            \eqqcolon \ket{\hat{\beta}_{ab}}\, .
        \]
        %where we have defined $\ket{\hat{\beta}_{ab}}$ to be the same state with hats.
        %
      Interestingly, using \cref{lemma:xzstore}, we also know that this is exactly 
        equivalent to the operation $\register[A] \xstore \register[B]$, which
        would yield the same Bell state as before except with $a$ and $b$ swapped:
        \[
          \ket{a, \hat b}\ \apply{\CXgate_{\register[BA]}}\
            \frac{\ket{a, 0} + (-1)^b \ket{a \xor 1, 1}}{\sqrt 2} =
            \ket{\beta_{ba}}\, ,
        \]
        which means that these two states must actually be the same state:
        \[
          \ket{\hat{\beta}_{ab}} = \frac{\ket{\hat 0, \hat b} + (-1)^a \ket{\hat 1, \hat b \xor \hat 1}}{\sqrt 2}
          = \frac{\ket{a, 0} + (-1)^b \ket{a \xor 1, 1}}{\sqrt 2} =
          \ket{\beta_{ba}}\, .
        \]
        What we have just discovered is another well-known fact, namely that the Hadamard
          ...
          without ever touching a double sum! 
        %\[
        ...
        With these additions, then, our \cref{tab:xzstore} is finally complete.
      ...
      \begin{table}[t]
        ...
        \rowcolors{2}{}{gray!10} % zebra striping
          \begin{tabular}{ccc}
            ...
            \midrule
            $\ket{a,b}$ & $\ket{a,b \xor a}$ & $\ket{a \xor b, b}$ \\
            $\ket{a,\hat b}$ & $(-1)^{a \cdot b}\ket{a, \hat b}$ & $\ket{\hat{\beta}_{ab}} = \ket{\beta_{ba}}$ \\
            $\ket{\hat a, b}$ & $\ket{\beta_{ab}}$ = $\ket{\hat{\beta}_{ba}}$ & $(-1)^{a \cdot b}\ket{\hat a, b}$ \\
    ...
    \begin{table}[t]
      ...
      %\resizebox{\textwidth}{!}{
        \begin{tabular}{ccc}
          ...
          $\ket{x,\hat y}$ & $(-1)^{x \cdot y}\ket{x, \hat y}$ & $\ket{\hat{\beta}_{xy}}$ \\
          $\ket{\hat x, y}$ & $\ket{\beta_{xy}}$ & $(-1)^{x \cdot y}\ket{\hat x, y}$ \\
  ...
  \subsection{Function Evaluation With $\xstore$ and $\zstore$}
    ...
    \begin{table}[h]
      \caption{
        ...
        Clearly, some of these expressions are more desirable to work with than
        others. Notice how letting $f$ to be the identity recovers
        \cref{tab:xzstore}.
      }
      \centering
      %\scriptsize
      ...
      %\resizebox{\textwidth}{!}{
        \begin{tabular}{ccc}
          ...
          $\ket{x,\hat y}$ & $(-1)^{y \cdot f(x)}\ket{x, \hat y}$ & 
            $\nicefrac{1}{\sqrt{2^n}} \sum_z (-1)^{x \cdot z}\ket{\hat z, \hat y \xor f(\hat z)}$
            \\
          $\ket{\hat x, y}$ & 
  ...
  \subsection{Demonstration}
    ...
    \subsubsection{The Deutsch--Jozsa Algorithm.}
      The Deutsch--Josza algorithm learns in a single function call whether a function
        $f: \bin^n \to \bin$ is constant or balanced (evaluates to $\bits 0$ and
        $\bits 1$ on exactly half of the inputs each), under the promise that it
        ...
        two function evaluations (or up to $\nicefrac{2^n}{2}$ in the worst case
        for the more general case) to arrive at the answer.
      ...
      The procedure is given in pseudocode in \cref{fig:DeutschJozsa}, shown in two
        ...
        which brings a slightly different perspective to the same basic
        operation.
      ...
      \begin{figure}[t]
   	    \begin{center}
              ...
              \underline{Deutsch--Jozsa}
  	  	  		\begin{nicodemus}
                \smallskip
                \item $\register[XB] \gets \ket{\hat{\bits{0}}^n, \hat{\bits{1}}}$
                \smallskip
            ...
            \quad 
              ...
              \underline{Alt.\ implementation}
  	  	  		\begin{nicodemus}
                \smallskip
                \item $\register[XB] \gets \ket{\bits{0}^n, \bits{1}}$
                \smallskip
      ...
      Recall that the operation $\register[B] \zstore f(\register[X])$ is
        ...
        discarding it. This gives us (as shown in \cref{tab:fxzstore}):
      \[
        \frac{1}{\sqrt{N}} \sum_{x \in \ZZ_{N}} 
          \ket{\hat{x}, \bits{1}}
        \apply{\hCZgate^{\otimes n}} 
        \frac{1}{\sqrt{N}} \sum_{x \in \ZZ_{N}} 
          (-1)^{f(x)} \ket{\hat x, \bits 1}\, .
      \]
      ...
      \[
        \frac{1}{\sqrt{N}} \sum_{x \in \ZZ_{N}} 
          (-1)^{f(x)} \ket{\hat x, \bits 1}\, .
        \frac{1}{\sqrt{N}} \sum_{x \in \ZZ_{N}} 
          (-1)^{c} \ket{\hat x, \bits 1}
        \equiv 
        \frac{1}{\sqrt{N}} \sum_{x \in \ZZ_{N}} 
          \ket{\hat x, \bits 1}
        = \ket{\bits{0}^n, 1}\, ,
      \]
      where we have factored out the global phase. Hence,
      measuring the $\register[X]$ register yields the outcome $\bits{0}^n$
    ...
    To see what happens in the case that $f(x)$ is \emph{balanced}, first
      ...
      \[
        ...
        = \frac{1}{\sqrt{2^n}} 
          ...
          \frac{1}{\sqrt{2^n}} 
          \sum_{x \in \ZZ_{N}}
          (-1)^{f(x) \xor x \cdot z} \ket{z, \bits 1}\, ,
      \]
      ...
      \[
        ...
        \eqqcolon \frac{1}{\sqrt{2^n}} 
          \sum_{z \in \ZZ_{N}} 
          \alpha_z \ket{z, \bits 1}\, ,
      \]
      then insert for $z=0$ to find the amplitude $\alpha_0$ that the outcome $\bits{0}^n$
      is observed:
      \[
      ...
      outcome $\bits{0}^n$ in the balanced case, which shows that $f$ is constant 
      \emph{if and only if} the Deutsch-Jozsa algorithm returns $0$.
    ...
    \subsubsection{Simon's Algorithm.}
      ...
      The main quantum step is in recovering a value $x_i$ which is guaranteed to
        satisfy $x_i \cdot s = 0$. This subroutine is then repeated until enough
      ...
      \begin{figure}[h]
   	    \begin{center}
              ...
              \underline{Simon's subroutine}
  	  	  		\begin{nicodemus}
                \smallskip
                \item $\register[XY] \gets \ket{\hat{\bits{0}}^n, \bits{0}^n}$
                \smallskip
            ...
            \quad
              ...
              \underline{Alt.\ implementation}
  	  	  		\begin{nicodemus}
                \smallskip
                \item $\register[XY] \gets \ket{\bits{0}^n, \hat{\bits{0}}^n}$
                \smallskip
      ...
      The quantum subroutine is shown in \cref{fig:Simon}. Let us see how it
        gives us an $x$ that is guaranteed to satisfy $x \cdot s = 0$.
      \begin{align*}
        &\ket{\hat{\bits{0}}^n, \bits{0}} 
          = \frac{1}{\sqrt{2^n}} \sum_{z \in \ZZ_N} \ket{z, \bits{0}} 
      ...
      \begin{align*}
        \apply{y \obstore \outReg}
          ...
          %f(z) = y\, .
      \end{align*}
      where $z$ satisfies that $f(z) = f(z \xor s) = y$.
      We will be applying a Hadamard-measurement, so translate this state to
      ...
      There are only two cases: If $x \cdot s = 1$, then $\hat{\alpha}_x = 1-1 = 0$, which means
      that the probability of observing such an outcome is also zero!
      Hence, for any $x$ we actually do observe, we must have that $x \cdot s =
      0$. Indeed, restricting to $x \cdot s = 0$, we see that we have the state
      \[
        \sum_{x\, :\, x \cdot s = 0}
        \frac{
          (-1)^{z \cdot x}
      ...
      \]
      which we can verify as a sanity check is normalized, since there are
      $2^{n-1}$ values of $x$, i.e.\ half of all $x$, that
      satisfy that $x \cdot s = 0$. (Otherwise, that would mean we must have
      done something wrong.)

    Applying a Hadamard-basis measurement to this state, then, is a procedure to 
      sample uniformly from the set of $x$ that satisfy $x \cdot s = 0$.
      ...
      \emph{same} $x$ when we later repeat the subroutine, so that with
      overwhelming probability we will only need to repeat the procedure $n$
      times to get $n$ independent values $x_i$ that all satisfy 
      $x_i \cdot s = 0$.

    \subsubsection{Bernstein--Vaszirani.}
      Dolor sit amet.
