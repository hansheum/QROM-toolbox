  ...
  \subsection{Dual-Alphabet Pseudocode}
    ...
    What are the equivalent operations for Hadamard-basis states? Well, we know
      ...
      Going back to multi-qubit states, recall that the operation 
        ...
        \]
        Denote this operation $\outReg \zstore \inReg$, to distinguish it from the
        bitwise CNOT operation $\outReg \zstore \inReg$.

    \subsubsection{Properties of $\xstore$ and $\zstore$.}
      We know that our operand $\zstore$ takes states $\ket{\hat x, \hat y}$ to 
        $\ket{\hat x, \hat y \xor \hat x}$, but what is its effect on computational
        basis states? To answer this question, let us go back to the
        single-qubit case: Let register $\register[A]$ be in state $\ket a$ and
        register $\register[B]$ be in state $\ket b$, where $a,b \in \bin$. 
        Then the operation $\register[B] \zstore \register[A]$ has the following effect:
      \[
        \ket{a,b} 
          ...
          = \ket{a \xor b, b}\, ,
      \]
      since the branch in which $\Zgate$ is applied picks up an extra factor
        $(-1)$ iff $b = 1$. Since the operations are applied bitwise, this
      ...
      \]
      In other words, applying $\zstore$ from $\inReg$ to $\outReg$ 
      \emph{has the same effect as} applying $\xstore$ from
      $\outReg$ to $\inReg$.

      It is easy to check that the dual statement also holds:
      If $\inReg$ is in state $\ket{\hat x}$ and $\outReg$ is in state
      $\ket{\hat y}$, then
      \[
        \outReg \xstore \inReg : \ket{\hat x, \hat y} \to \ket{\hat x \xor \hat y, \hat y}
        \quad\equiv\quad
        \inReg \zstore \outReg : \ket{\hat x, \hat y} \to \ket{\hat x \xor \hat y, \hat y}\, .
      \]
      In fact, since we have shown (twice) that the equivalence holds for every
        basis state, this suffices to prove the general statement.
      
      \begin{lemma}\label{lemma:xzstore}
        Let $\inReg$ and $\outReg$ be equally-sized qubit registers. Then
      ...
      What about mixed-alphabet states like 
