\section{The Compressed-Oracle Technique: Or, How to Lazy-Sample a QRO}\label{sec:compressed}
  ...
  \subsection{Setup}
    \begin{table}[t]
      ...
      \caption{
        \label{tab:notation}
        An overview of our notation for compressed oracles.
      }
      %
      \rowcolors{2}{}{gray!10}
      \begin{tabular}{cccc}
        ...
        \midrule
        Input register & $\inputreg$ & $n$-sized register \\
        Output register & $\outputreg$ & $m$-sized register \\
        Database & $\database$ & $(q \cdot (1 + n + m))$-sized register \\
        \midrule
        Empty string & $\emptystring$ & $0 \concat 0^n$ \\
        Interpreted string & $x$ & $0 \concat x,\ x \in \bin^n$ \\
        Invalid string & $\slashed{x}$ & $1 \concat x,\ x \in \bin^n \backslash \{0^n\}$ \\
        Hadamard-domain value & $\hat{y}$ & Label for basis state in the support of $\Hgate^{\otimes m} \ket{y}$ \\
        \midrule
        List & $\Dlist$ & List of $q$ entries \\
        Entry & $(x, \hat y)$ & $(1 + n, m)$-sized string tuple $(b_\emptystring \concat x, \hat y)$ \\
        Initialized entry & $(\emptystring,0^m)$ & $(1 + n, m)$-sized string tuple \\
        $x$ is in list & $\Dlist[x] = \hat{y}$ & $x \in \bin^n, \hat{y} = \bin^m$ \\
        $x$ is not in list & $\Dlist[x] = \perp$ & Special symbol \\
        \midrule
        Input subregister & $\database_\inputreg$ & $(q \cdot (1 + n))$-sized register storing input values \\
        Output subregister & $\database_\outputreg$ & $(q \cdot m)$-sized register storing output values \\
        Entry subregister & $\database_i$ & $(1+n+m)$-sized subregister of
        $\database$ \\ && storing $i$th list entry \\
        Specified entry subregister & $\database(x)$ & $(1+n+m)$-sized
        subregister of $\database$ \\ && storing entry containing $x$ in current branch \\
        \bottomrule
      \end{tabular}
    \end{table}
