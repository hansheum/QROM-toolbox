\section{Lazy-sampling QROs: The Compressed Oracle Technique}
  ...
  \subsection{Playing with CROs}
    ...
    \subsection{Extracting Queries}\label{sec:extracting}
      ...
      \begin{theorem}[Extraction is imperfect]\label{thm:perfect}
        ...
        After $\adv$ halts, let $\extractor$ first move
          ...
          Let $\advantage{}{\rel}[\extractor^\adv]$ be the probability that
        \begin{enumerate}
          \item For all tuples $(x,y)$, $\Dlist[x] = y \neq\ \perp$; and
          \item The collection of tuples $\{(x,y)\}$ is contained in $\rel$.
        \end{enumerate}

        \noindent
        Then we have that, for all adversaries $\adv$,
        \[
          \sqrt{\advantage{}{\rel}[\adv^{\RO}]} \leq
          \sqrt{\advantage{}{\rel}[\extractor^\adv]} +
          \sqrt{\frac{\ell}{2^m}}\, ,
        \]
        which implies the slightly looser (but perhaps more practical) bound
        \[
          \advantage{}{\rel}[\adv^{\RO}] \leq
          \advantage{}{\rel}[\extractor^\adv] +
