  ...
  \subsection{Cryptographic Primitives}
    ...
    Our definitions are slightly simplified in that they leave parameter
      generation implicit.\footnote{
        ...
        generation to the top of every game.
      }
      ...
      exhibit a small correctness error, something their security proofs will
      then have to account for~\cite{Any number of recent works on this}.

    \subsubsection{Public-Key Encryption.}
      The quest for asymmetric cryptography began with Diffie and Hellman's idea
        of public-key encryption~\cite{DifHel76}, so fittingly this is where we
        begin too.
      ...
      We will also use the convention that if you know a secret key $\sk$ belonging to a
        ...
        keypair to instead be $(\pk, (\pk, \sk))$; but it is not w.l.o.g.\ in
        the sense that it enforces a unique-public-per-secret-key relation,
        and there exist PKE schemes that violate this.\footnote{
          Some security proofs even rely on the non-uniqueness of
          keypairs!~\cite{EC:BelHofYil09}
        }
      ...
      \noindent
      i.e., there are no ciphertexts in $\CSpace$ that successfully decrypt
    ...
    \subsubsection{Digital Signature Algorithms.}
      \hhnote{Should I add sigma protocols and NIZKs too?}

    \subsubsection{Hash Functions.}

  \subsection{Security}
    \subsubsection{Games and Advantages.}
      \hhnote{copy-pasted and slightly outdated}
