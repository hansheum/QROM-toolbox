\section{Basics of Cryptography}\label{sec:basics}
  ...
  \subsection{Security}
    \subsubsection{Games and Advantages.}
      Security is defined in terms of a security game (or \emph{experiment})
        ...
        (in which case $\experiment{}{\pcalgostyle{P}}$ outputs $1$), then we say that
        $\adv$ broke the security of $\pcalgostyle{P}$.
        If there exists an efficient adversary (i.e., an attack algorithm) that wins the game with 
        significant \emph{advantage}, then we say that $\pcalgostyle{P}$ is
        \emph{broken}.

      How advantages are defined depends on what kind of security notion we are talking
        about. Usually, we are only concerned with two kinds of notions:
        \emph{computational}, in which case the adversary is tasked with
        recovering some secret value, and \emph{distinguishing}, in which case
        the adversary should distinguish between two known cases. For the former
        case, the advantage is simply the probability that the adversary wins.
        For the latter case, however, the adversary can win with probability
      ...
      \hhnote{outdated below}

      The standard notion of security for public-key encryption is usually taken to
    ...
    \subsubsection{Asymptotic versus Concrete.}
      \hhnote{copy-pasted}
