...
\newif\ifMarginsForTodo % adapts margins for non-inline todonotes
\MarginsForTodotrue
%\MarginsForTodofalse
...
\newcommand{\khnote}[2][]{\todo[color=violet,textcolor=white]{\small\textbf{K:} \if!#1! #2 \else (#1)\\ #2\fi}\xspace}
...
% Appendices are just sections after \appendix, so cleveref would call them
% "Sect. A". The fix is \crefalias (see after \appendix in the body): labels
% set there are recorded under these appendix-specific types instead.
\crefname{appendix}{App.}{App.}             \Crefname{appendix}{App.}{App.}
\crefname{subappendix}{App.}{App.}          \Crefname{subappendix}{App.}{App.}
\crefname{subsubappendix}{App.}{App.}       \Crefname{subsubappendix}{App.}{App.}
...
% Change the paper format so I can do non-inline comments
\ifMarginsForTodo 
...
\begin{document}
  ...
  \appendix
  % llncs does not reset hyperref's anchor name at \appendix, so appendix
  % section anchors collide with body ones ("section.1") and the duplicate is
  % silently dropped -- appendix links then jump to the body section with the
  % same number. Give them their own namespace:
  \renewcommand{\theHsection}{app.\Alph{section}}
  % From here on, section-level labels are of cleveref type "appendix" etc.,
  % so \cref renders "App. A" rather than "Sect. A". Must stay at top level
  % of the document body (\crefalias is group-local).
