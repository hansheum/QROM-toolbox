\section{Preface}
  ...
  \subsubsection{Outline.}
    We start in \autoref{part:prelims} by laying the foundations, with
      definitions for basic crypto concepts in \autoref{sec:basics}, before
      introducing the reader to basic quantum computing concepts in \autoref{sec:primer}
      (we only assume knowledge of basic linear algebra). Then in
      \autoref{sec:rom} we introduce the random oracle model, and show how to
      update the formalism to support quantum queries.
      We will build some intuition, trying to get a
      ``feel'' for QROs and how to (and how not to) reason about them.
