\section{Tool 1: Quantum Programming with the O2H Toolkit}\label{sec:o2h}
  \todoenum[rough structure]{
    \item (assuming the classical proofs were shown in previous sections)
    \item From OW-CPA to IND-CPA via the U(check)-transformation
    \begin{enumerate} 
      \item First via Unruh's original O2H
      \item then tighter via the semi-classical ``find'' O2H
      \item then tighter in one direction via the double-sided O2H
      \item then tighter in another direction via Measure-Rewind-Measure
      \item and finally (since we assue dPKE) Measure-Rewind-Extract
    \end{enumerate}
    \item Introduce the fixed-permutation formulation? 
    \item From NMA NIZK to (s?)EUF-CMA signature via adaptively-reprogrammable O2H
  }
  ...
  \begin{table}[h]
    ...
    \begin{tabular}{ccccccccccc}
      ...
      %  $\nicefrac{1}{\varepsilon} \approx \qRP^2 \qRO$ & \cref{lemma:resampling-fp} & \cite{EPRINT:Jaeger24b} \\
      %\midrule
      \cite{PKC:PanZen24} &
  ...
  \end{table}
  \hhnote{
    Add dot for the double-sided requirement, which MRM (and MRE?) also comes
    with, with the single-sided case still an open question?
  }
