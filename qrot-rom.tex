...
\section{The Random Oracle Model}\label{sec:rom}
  ...
  \subsection{One-way to Hiding}\label{sec:C-O2H}
    \autoref{thm:cparom} may be about a specific construction (the
    ...
    It may not be clear at a glance how this lemma relates to the proof of
      \autoref{thm:cparom}. After all, there there was only one random oracle in
      play, while \cref{lemma:co2h-find-single} has two. Also,
      \autoref{thm:cparom} speaks of an adversary and a reduction, while
    ...
    Taken together, we can now provide simpler proof of the crucial hop
      ...
      whatever value $\extractor$ outputs once it halts.
    ...
    \begin{exercise}\label{ex:cparom-lossy}
      Give an alternative, simpler proof of \cref{thm:cparom-guess} by
      referencing \cref{lemma:co2h-guess-single}. \\
      \emph{
        Hint: Let $\extractor$ do the heavy lifting; the reduction $\bdv$ only
        needs to provide $\extractor$ with its expected input.
      }
    \end{exercise}
  ...
  \subsection{The Post-Quantum Setting -- What Breaks?}
    Having seen the proofs of \cref{thm:cparom} and \cref{thm:cparom-guess}, 
      \cref{lemma:co2h-find-single}
      and \cref{lemma:co2h-guess-single} may seem
      almost too trivial to bother writing down. After all \ldots 

    However, the same is certainly not true for \ldots
      where this very same formalism will turn out to be among the most useful
      tools we have. In fact, quantum O2H lemmas show up everywhere in QROM
      security proofs. We will meet our first quantum O2H
      lemmas in \cref{sec:o2h}, where we will use them to upgrade
